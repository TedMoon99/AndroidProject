...
// Thread에서 처리하는 작업에 오류가 발생하면 Thread가 준당된다.
// Thread는 동시에 처리하고자 하는 작업이 있거나
// 오류가 발생할 가능성이 있는 작업을 처리할 때 사용한다.

// UI Thread, origianl Thread : 안드로이드 OS가 발생시킨 쓰래드. 화면 갱신, 생명주기와 관련된 메서드 호출, 리스너 처리 등의 일을 담당하게 된다.
// 이 쓰래드가 호출하는 메서드 내부의 코드는 무조건 금방 끝나야 한다.
// 사용자 쓰래드 : 개발자가 thread 코드 블럭을 이용해 발생시키는 쓰랟. 오래 걸리는 작업이나, 오류가 발생될 가능성이 있는 작업을 담당하게 한다.
// Ex) 네트워크, 지속적으로 반복해야 하는 작업

// runOnUiThread 코드 블럭 : 사용자 쓰래드 안에서 화면에 관련된 작업이 필요한 경우 화면과 관련된 코드를
...
class MainActivity : AppCompatActivity() {
    ...
    // 사용자 Thread는 Activity가 종료되었다고 하더라도 계속 동작을 한다.
    // OS가 판단했을 때 시스템 메모리가 부족한 경우 사용자 Thread들이 모두 종료된다.(이를 방지하기 위해서는 Service를 사용해야 한다.)
    // 따라서 Activity가 종료딜 때 발생시킨 쓰래드들도 종료되게 해야 한다.
    ...
    override fun onDestroy() {
        ...
        // Activity가 종료될 때 Thread 내부의 코드가 종료될 수 있도록
