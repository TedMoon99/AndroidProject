...
import kr.co.lion.question3.databinding.ActivityMainBinding
...
문제3)
ToDo 리스트 어플을 만든다.
첫 번째 화면의 툴바에는 추가 메뉴가 있다.
첫 번째 화면에는 리사이클러 뷰가 있다.
리사이클러 뷰에는 할일 목록이 있으며
완료한 일의 항목 글자색은 회색으로
완료하지 않은 일의 항목은 검정색으로 표시한다.

추가 메뉴를 누르면 할일을 입력하는 화면이 나타난다.
할일을 입력하는 화면에는 할일 제목과 할일의 내용을 입력받는다.


수정 메뉴를 누르면 수정화면이 나타나며 할일 제목과 내용을 수정할 수 있도록한다.
삭제 메뉴를 누르면 삭제하고 첫 번째 화면으로 돌아간다.
완료 메뉴를 누르면 완료 처리를 하고 첫 번째 화면으로 돌아간다.

모든 화면은 프래그먼트로 구성한다.
모든 데이터는 SQLiteDatabase에 저장하여 사용한다.
 */
